...
const updateUser = async (req, res) => {
    try {
        ...
        //---------(Get data)
        let { fname, lname, email, password, phone, address, ...anotherval } = req.body;
        let data = req.body;
        ...
        if (validators.isValidRequestBody(anotherval)) return res.status(400).send({ status: false, message: 'Cannot update this detail ' })
       ...
       //  let updateObject = {};
        ...
        if ("lname" in data) {
            ...
            if (!data.lname.match(/^[A-Za-z ]+$/)) {
                return res.status(400).send({
                    status: false, message: "last Name should contain only alphabets",
                });
            }
            //  data.lname = lname;
        }
        ...
        if ("phone" in data) {
            ...
            let mobileAlreadyExists = await userModel.findOne({ phone: phone });

            if (mobileAlreadyExists)
                return res.status(400).send({
                    status: false, message: "Phone number has already been used.",
                });
         //   data.phone = phone;
        }
        ...
        if ("password" in data) {
            ...
            if (!validators.isValidPassword(password))
                return res.status(400).send({
                    status: false, message: "Password should consist a minimum of 8 characters and a maximum of 15 characters.",
                });
        ...
        if (profileImage) {
            if (!validators.isvalidImage(profileImage))
                return res.status(400).send({
                    status: false, message: "Image should be in the format of jpg, png, jpeg",
                });
            let uploadedImage = await aws.uploadFile(profileImage);
        ...
        //---------(Upadate Document)
        let update = await userModel.findOneAndUpdate({ _id: userRequesting }, data, { new: true }
        );
        //---------(Send Response)
